...
def main():
    parser = argparse.ArgumentParser(description="CDR3 generation from peptide-HLA prompts.")
    parser.add_argument('-n', '--n', type=int, required=True, help='Number of antibodes')
    parser.add_argument('-o', '--output', type=str, required=True, help='Output file')
    parser.add_argument('-p', '--peptide', type=str, required=True, help='Peptide sequence for the prompt')
    parser.add_argument('-H', '--hla-sequence', type=str, required=True, help='Full HLA sequence for the prompt')
    ...
    required_special_tokens = ['[PEP]', '[HLA]', '[SEP]']
    missing_tokens = [token for token in required_special_tokens if token not in tokenizer.get_vocab()]

    if missing_tokens:
        raise ValueError(f"Tokenizer missing required special tokens: {', '.join(missing_tokens)}")
    ...
    prompt = args.peptide + '[PEP]' + args.hla_sequence + '[SEP]'
